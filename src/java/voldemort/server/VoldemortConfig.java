...
public class VoldemortConfig implements Serializable {
    ...
    private void initializeStateFromProps() throws UndefinedPropertyException {
        ...
        this.sslEnabled = this.allProps.getBoolean("ssl.enable", false);
        this.sslProtocol = this.allProps.getString("ssl.protocol", "TLSv1.2");
        String cipherSuites = this.allProps.getString("ssl.ciphersuites", null);
        if(cipherSuites != null && !cipherSuites.isEmpty()) {
            this.sslCipherSuites = cipherSuites.split(",");
        }
        this.sslKeyStore = this.allProps.getString("ssl.keystore", null);
        this.sslKeyStorePassword = this.allProps.getString("ssl.keystore.password", null);
        this.sslTrustStore = this.allProps.getString("ssl.truststore", null);
        this.sslTrustStorePassword = this.allProps.getString("ssl.truststore.password", null);

        this.nioAdminConnectorSelectors = this.allProps.getInt("nio.admin.connector.selectors",
                                                       Math.max(8, Runtime.getRuntime()
                                                                          .availableProcessors()));
        this.nioAdminConnectorKeepAlive = this.allProps.getBoolean("nio.admin.connector.keepalive", false);
        // a value <= 0 forces the default to be used
